...
def _word_bytes(res: dict) -> bytes | None:
    """
    Genera la Memoria de Cálculo Word (NOM-081) usando generar_memoria.js.
    Requiere Node.js instalado y generar_memoria.js en el mismo directorio.
    Devuelve None si Node.js no está disponible.
    """
    import json, subprocess

    js_path = Path(__file__).parent / "generar_memoria.js"
    if not js_path.exists():
        return None

    # Serializar resultados al JSON que espera el script JS
    ...
    payload = _make_serializable(res)
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".docx") as df:
        docx_path = df.name

    try:
        result = subprocess.run(
            ["node", str(js_path), json_path, docx_path],
...
with tab_resultados:
    ...
    base_name = meta_sitio.replace(" ", "_") if "meta_sitio" in dir() else "NOM081"
    ...
    with col_dl2:
        # Ensamblar el dict completo que espera generar_memoria.js
        import copy
        ...
        for p in estudio.per_fuente:
            res_full["fuente_stats"][p]["suma"] = round(
                float(estudio.df_fuente[p].sum()), 2)
        for p in estudio.per_fondo:
            res_full["fondo_stats"][p]["suma"]  = round(
                float(estudio.df_fondo[p].sum()),  2)

        res_full["promedios"] = {
            "fuente": {
                "N50":   res["fuente"]["N50_prom"],
                "sigma": res["fuente"]["sigma_prom"],
                "N10":   res["fuente"]["N10_prom"],
                "Neq":   res["fuente"]["Neq_eq"],
            },
            "fondo": {
                "N50":   res["fondo"]["N50_prom"],
        ...
        if docx_data:
            ...
            st.caption(
                "Documento Word estilo técnico NOM-081 con: ficha de identificación, "
                "tablas de 35 lecturas, cálculos por periodo, fórmulas explicadas, "
                "correcciones y resultado final."
            )
        else:
            st.info(
                "⚠ La exportación Word requiere **Node.js** instalado en el servidor. "
                "Disponible al ejecutar la app localmente con Node.js instalado.",
                icon="ℹ️",
            )
