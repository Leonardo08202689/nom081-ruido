...
def _set_col_width(cell, width_cm: float):
    ...
    # OOXML requiere tcW como primer elemento de tcPr
    if len(tcPr):
        tcPr.insert(0, tcW)
    else:
        tcPr.append(tcW)
...
def _tabla_correcciones(doc, corr, prom):
    ...
    n50c_val = f"{corr['N50_corr']:.2f}"  # N'50 siempre aplica; solo Cf es condicional
...
def generar_word(resultado: dict) -> bytes:
    ...
    if not inserted:
        pPr.append(shd)
    run = p.add_run(f"CÁLCULOS PARA {meta.get('zona', 'ZONA CRÍTICA').upper()}")
